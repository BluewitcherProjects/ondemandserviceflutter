...
class AuthController extends GetxController implements GetxService {
  ...
  Future<void> _saveTokenAndNavigate({String? fromPage, required String token, String? emailPhone, String? password}) async {
  ...
  Future<ResponseModel?> sendVerificationCode({required String identity, required String identityType,required  SendOtpType type, int checkUser = 1, String fromPage = "", bool isResend = false}) async {
    ...
    if(type == SendOtpType.firebase){
       _sendOtpForFirebaseVerification(identity: identity, identityType: identityType, fromPage: fromPage, isResend: isResend);
    } else if(type == SendOtpType.verification){
      responseModel = await _sendOtpForVerificationScreen(identity: identity, identityType: identityType, checkUser: checkUser, isResend: isResend);
    }else{
  ...
  Future<void> _sendOtpForFirebaseVerification({required String identity, required String identityType, required String fromPage, bool isResend = false}) async {
    if(!isResend) {
      _isLoading = true;
      update();
    }
